...
coory:  (0, coorH)
u:      (-pi, pi)
v:      (-pi/2, pi/2) 反转 y，0 -> pi/2，H -> -pi/2
"""
...
def np_coor2xy(coor, z=50, coorW=1024, coorH=512):
    ...
    '''
    coor = np.array(coor)
    u = np_coorx2u(coor[:, 0], coorW)  # (-pi, pi)
    v = np_coory2v(coor[:, 1], coorH)  # (-pi/2, pi/2) 反转 y，0 -> pi/2，H -> -pi/2
    # 转化成地平面的 xy 坐标
    c = z / np.tan(v)  # 角点在 3D 空间到原点的距离
    x = c * np.sin(u)  # 转化为 3D x 坐标
    y = -c * np.cos(u)  # 转化为 3D y 坐标
    return np.hstack([x[:, None], y[:, None]])  # (4,) -> (4, 1) -> 水平堆叠 -> (4, 2)
